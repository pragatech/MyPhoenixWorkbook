defmodule Rumbl.AccountsTest do
  ...
  describe "register_user/1" do
    ...
    test "requires password atleast 6 characters long" do
  ...
  describe "authenticate_by_username_and_pass/2" do
    ...
    test "returns unauthorized error with invalid password", %{user: user} do
      assert {:error, :unauthorized} =
              Accounts.authenticate_by_username_and_pass(user.username, "bad-pass")
    end

    test "returns not found error with no matching user for email" do
      assert {:error, :not_found} =
             Accounts.authenticate_by_username_and_pass("unknownuserr", @pass)
    end
  end
end
