defmodule RumblWeb.VideoControllerTest do
  ...
  test "authorizes actions against access by other users", %{conn: conn} do
    ...
    non_owner = user_fixture(username: "aishu")
  ...
  describe "with a logged in user" do
    ...
    @tag login_as: "max"
    test "lists all users video on index", %{conn: conn, user: user} do
    ...
    @tag login_as: "max"
    test "does not create video vid, renders error when invalid", %{conn: conn} do
